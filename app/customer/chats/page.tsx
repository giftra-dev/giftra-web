...
function CustomerChatsContent() {
  const { currentUser, chatRooms, requests, artistProfiles, messages } = useGiftraStore()
          ...
          {myChats.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {myChats.map((chat) => {
                ...
                const artist = chat.artistId 
                  ? artistProfiles.find(a => a.id === chat.artistId)
                  : null
                ...
                return (
                  <Link
                    key={chat.id}
                    href={chat.isLocked ? `/customer/request/${chat.requestId}` : `/chat/${chat.id}`}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="font-medium truncate">
                          {artist?.name || "Waiting for artist"}
                        </h3>
